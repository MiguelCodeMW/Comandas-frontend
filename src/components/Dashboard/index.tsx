...
import LoadingSpinner from "../LoadingSpinner/LoadingSpinner";
import ErrorMessage from "../ErrorMessage/ErrorMessage"; // Asegúrate de que esta importación sea correcta (ya la corregimos antes)
import Button from "../Button/Button";
import styles from "./Dashboard.module.css";
import { NAMES } from "../../utils/Constants/text";
import { ROUTES } from "../../utils/Constants/routes";
...
function Dashboard() {
  const {
    ...
    iva,
    moneda,
    user, // El tipo 'user' se obtiene del hook useDashboard. Su tipo debería estar en UserTypes.ts
    handleLogout,
    handleIvaGuardado,
  ...
  return (
    <div className={styles.container}>
      ...
      <DashboardComandasList
        ...
        mostrarPagadas={mostrarPagadas}
      />
      {/* Si hay un error pero ya se mostraron comandas (ej. error al actualizar, no en carga inicial) */}
      {error && comandasFiltradas.length > 0 && (
        <p className={styles.inlineError}>{error}</p>
      )}
    </div>
